...
if __name__ == "__main__":
    ...
    runs = Runs()
    for cell in (Cells() & dict(cell_type='p-unit', cell_id="2014-12-03-ai")).fetch.as_dict:
        ...
        for contrast in [5, 10, 20]:
            ...
            if target_trials:
                with Figure02(filename=generate_filename(cell, contrast=contrast)) as (fig, ax):
                    ...
                    ax['spectrum'].plot(eod_freq, y[:-1], '-', zorder=-1, lw=1, color=colors[1], label='EOD')
                    ax['spectrum'].plot(stim_freq, y[:-1], '-', zorder=-1, lw=1, color=colors[0],
                                        label='stimulus')
                    ax['spectrum'].plot(np.abs(deltaf_freq), y[:-1], '-', zorder=-1, lw=1, color=colors[3],
                                        label=r'$|\Delta f|$')
